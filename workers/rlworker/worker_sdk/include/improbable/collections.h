...
#ifndef IMPROBABLE_WORKER_SDK_INCLUDE_IMPROBABLE_COLLECTIONS_H
#define IMPROBABLE_WORKER_SDK_INCLUDE_IMPROBABLE_COLLECTIONS_H
#include <cstddef>
#include <iterator>
#include <list>
#include <memory>
...
public:
  ...
  /** Iterator to one past the first element. */
  iterator end();
  ...
  /** Reverse iterator to one past the first element. */
...
private:
  static constexpr bool is_enum = std::is_enum<key_type>::value;
  using hash_type = typename std::conditional<is_enum, detail::EnumHash<K>, std::hash<K>>::type;

  std::list<value_type> values;
  std::unordered_map<key_type, iterator, hash_type> lut;
};
...
#include "detail/collections.i.h"
#endif  // IMPROBABLE_WORKER_SDK_INCLUDE_IMPROBABLE_COLLECTIONS_H
