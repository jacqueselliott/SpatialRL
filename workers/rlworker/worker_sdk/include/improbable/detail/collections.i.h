...
#ifndef IMPROBABLE_WORKER_SDK_INCLUDE_IMPROBABLE_DETAIL_COLLECTIONS_I_H
#define IMPROBABLE_WORKER_SDK_INCLUDE_IMPROBABLE_DETAIL_COLLECTIONS_I_H
...
struct Contains<T, A, Args...> {
  static constexpr const bool value = std::is_same<T, A>::value || Contains<T, Args...>::value;
};
...
template <typename T>
std::size_t EnumHash<T>::operator()(T value) const {
...
: stored_type{other.stored_type}, storage{other.storage->copy()} {}
...
template <typename T, typename>
T* Variant<Args...>::data() {
  return *stored_type == typeid(T) ? &static_cast<Storage<T>*>(storage.get())->value : nullptr;
}
...
#endif  // IMPROBABLE_WORKER_SDK_INCLUDE_IMPROBABLE_DETAIL_COLLECTIONS_I_H
